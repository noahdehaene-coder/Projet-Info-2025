...
export class AdeService {
  ...
  async getMatieres() {
    ...
    await fetch(
      `lienADEsessionId=${sessionId}&function=setProject&projectId=8`,
    );

    const response = await fetch(
      `lienADEsessionId=${sessionId}&function=getActivities&tree=TRUE`,
    );
  ...
  async getCreneaux(date: string): Promise<any[]> {
    ...
    const eventsResponse = await fetch(
      `lienADEsessionId=${sessionId}&function=getEvents&date=${formattedDate}`,
    );
